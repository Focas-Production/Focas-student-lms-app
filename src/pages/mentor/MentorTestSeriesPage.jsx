    ...
    <div className="bg-white rounded-2xl shadow-sm p-4 flex items-center gap-4">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-500 mt-0.5">
          {sub.studentName || 'Student'} · {sub.level || '—'} · {sub.subject} · {sub.chapter || '—'}{sub.unit ? ` · ${sub.unit}` : ''}
        </p>
...
function EvaluateModal({ sub, onClose, onDone }) {
  ...
  // Answer key opens in a separate browser tab so the mentor can grade side-by-side.
  const openAnswerKey = async () => {
    const win = window.open('', '_blank')
    setKeyLoading(true)
    try {
      const { url } = await apiFetch(`/api/mentor/test-series/${sub._id}/answer-key`)
      if (win) win.location = url
      else window.location.href = url
    } catch (e) {
      if (win) win.close()
      setError(e.message || 'Answer key not available')
  ...
  const submit = async () => {
    ...
    try {
      ...
      await apiFetch(`/api/mentor/test-series/${sub._id}/evaluate`, {
        method: 'POST',
        body: JSON.stringify({ awardedMarks: m, mentorNotes: notes, reviewVideoUrl: reviewVideo, files: evaluatedFiles }),
      })
      onDone()
    } catch (e) {
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto">
          <div className="bg-gray-50 rounded-xl px-4 py-3 text-sm">
            ...
            <p className="text-xs text-gray-500 mt-0.5">{sub.studentName} · {sub.level} · {sub.subject} · {sub.chapter}{sub.unit ? ` · ${sub.unit}` : ''}</p>
            <p className="text-xs text-gray-400 mt-0.5">{sub.testDuration} min · {sub.totalMarks} marks</p>
            {sub.hasAnswerKey && (
              <button onClick={openAnswerKey} disabled={keyLoading}
                className="mt-2 inline-flex items-center gap-1 text-xs font-semibold bg-emerald-100 text-emerald-700 rounded-lg px-3 py-1.5 hover:bg-emerald-200 disabled:opacity-50">
                ...
                {keyLoading ? 'Loading…' : 'View Answer Key ↗'}
              </button>
            )}
          </div>
          ...
          <div>
            ...
            {readOnly ? (
              reviewVideo
                ? <a href={reviewVideo} target="_blank" rel="noreferrer"
